...
void TrackerHit::Print()
{
	...
	G4double theta = atan(y/x) * (180.0/M_PI);
	G4double r = sqrt(pow(x,2)+pow(y,2));
	G4double phi = atan(r/z) * (180.0/M_PI);


  G4cout
     << "  trackID: " << fTrackID << " Theta Ring: " << fChamberNb
  ...
  //std::ostringstream outFile("output.txt");

 // outFile<<"TrackID: " << fTrackID << "Theta: " 
  //<< fChamberNb << "Edep: " << std::setw(7) << G4BestUnit(fEdep,"Energy")
  //<<G4endl;
